...
const wordsSlice = createSlice({
	...
	reducers: {
		toggleDifficult(state, action: ActionType<{ id: number }>) {
			const newDifficults = _.xor(state[action.payload.space].difficults, [
				action.payload.id,
			]);
			state[action.payload.space].difficults = newDifficults;
			if (newDifficults.length === 0)
				state[action.payload.space].onlyDifficult = false;
			LSHelpoers.setItem(`${action.payload.space}-difficults`, newDifficults);
		},
		toggleOnlyDifficult(state, action: ActionType<{}>) {
			const newOnlyDifficult = !state[action.payload.space].onlyDifficult;
			if (newOnlyDifficult) state[action.payload.space].selectedGroups = [];
			state[action.payload.space].onlyDifficult = newOnlyDifficult;
		},
		...
		toggleSelectedGroups(state, action: ActionType<{ groupid: number }>) {
			state[action.payload.space].selectedGroups = _.xor(
				state[action.payload.space].selectedGroups,
